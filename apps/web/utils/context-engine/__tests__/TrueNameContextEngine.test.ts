...
'context_not_found_or_no_assignment',
  );
...
insert: vi.fn().mockResolvedValue({ data: null, error: null }),
  ...
  expect(result.source).toBe('preferred_fallback');
  expect(result.metadata.fallbackReason).toBe(
'no_specific_request_with_database_error',
  );
...
{ targetUserId: 'user-3', requesterUserId: 'requester-1' },
  ...
  expect(results.every((r) => r.source === 'preferred_fallback')).toBe(
true,
  );
});
