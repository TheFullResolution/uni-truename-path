-- CRITICAL FIX: Add missing visibility field to complete_signup_with_oidc function
-- Issue: Function fails due to default_context_visibility_check constraint
...
DO $$
BEGIN
RAISE LOG 'TrueNamePath Critical Fix Part 2: Adding visibility field to complete_signup_with_oidc function';
END
$$;

-- Drop and recreate the function with visibility field
DROP FUNCTION IF EXISTS complete_signup_with_oidc(UUID, TEXT, TEXT, TEXT, TEXT, TEXT);

-- Create the fully corrected function
...
-- FIXED: Add visibility field to satisfy default_context_visibility_check constraint
-- Permanent contexts must have visibility = 'public'
INSERT INTO user_contexts (
  user_id, context_name, description, is_permanent, visibility
) VALUES (
  p_user_id, 'Default', 'Default identity context', TRUE, 'public'
...
RAISE LOG 'Using existing default context: %', v_default_context_id;
  ...
  -- Create OIDC assignments using the correct table structure
...
-- Add comprehensive comment documenting all fixes
COMMENT ON FUNCTION complete_signup_with_oidc IS 
'FULLY FIXED (Aug 21, 2025): Updated for Step 15.7.1 schema changes. 
...
3) Added visibility field to satisfy default_context_visibility_check constraint,
4) Added comprehensive error handling and logging for debugging, 
5) Maintains all original functionality with new table structure';

-- Log successful migration completion
DO $$
BEGIN
RAISE LOG 'TrueNamePath Critical Fix Part 2 Complete: complete_signup_with_oidc function now includes visibility field';
END
$$;